...
t('chat rieng 1-1: moi tin thanh viec (trigger dm)', () => {
  const msgs = [{ id: 50, platform: 'telegram', isPrivate: true, chatId: 'u1', chatName: 'Anh Bao', text: 'gui giup anh file di', date: '2026-07-18T00:00:00Z' }];
  const tasks = extractTasks(msgs, identity, { matchPrivate: true });
  assert.strictEqual(tasks.length, 1);
  assert.ok(tasks[0].triggers.includes('dm'));
});

t('chat rieng: tat matchPrivate thi khong bat', () => {
  const msgs = [{ id: 51, platform: 'telegram', isPrivate: true, chatId: 'u1', chatName: 'Anh Bao', text: 'chao em', date: '2026-07-18T00:00:00Z' }];
  assert.strictEqual(extractTasks(msgs, identity, { matchPrivate: false }).length, 0);
});
